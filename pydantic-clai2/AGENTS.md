...
| `compaction.py` | the built-in `compaction` plugin: harness `TieredCompaction` over `FallbackCompaction([SummarizingCompaction, SlidingWindowCompaction])`, `/compact`, the context alert |
