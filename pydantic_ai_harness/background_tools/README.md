...
[deferred tools](https://ai.pydantic.dev/deferred-tools/) or ends through cancellation, a usage limit,
or an error, live tasks are cancelled and their results are dropped. Run cleanup waits for the
cancelled tasks to finish, also when the run itself is cancelled by an outer anyio cancel scope, so
async tools must propagate cancellation. Suppressing cancellation can keep cleanup open.
