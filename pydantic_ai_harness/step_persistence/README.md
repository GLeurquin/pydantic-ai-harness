...
A user-facing list of past dialogues (a chat shell's "resume" picker, say)
is `list_runs()` grouped by `conversation_id`. The first run in a group
carries whatever the capability's `metadata` said when the dialogue started,
and the last run's `latest_snapshot` is where it left off. Two store
methods cover the edits such a catalogue needs on top of the append-only
log:
...
Events already written keep the metadata they were written with;
`update_run_metadata` changes the `RunRecord` only.
