...
Legacy-key reads are retained, including in 0.28.0. `Budget(window='total')` and budgets with `retain='forever'` can leave counters under the old name indefinitely; removing those reads would subtract that spend from the enforced total. The store reads opaque keys without budget or retention metadata, so it keeps the extra read for every key rather than guessing which counters can be discarded. No migration is required for an upgrade. Do not delete an old counter merely because all workers have upgraded: its value still contributes to the total. Expiring old keys retain the expiry described above.
...
carries no token, is deprecated. A store of that shape still works, through
