"""Tests for the filesystem tools, exercised through the run's workspace."""
...
class _LocalFilesystemBackend:
    def __init__(self, backend: LocalWorkspace) -> None:
...
async def test_a_relative_root_resolves_against_the_sandbox_working_directory(
    ...
    result = await _call(_toolset(Path('workspace')), _ctx(workspace), 'read_file', {'path': 'a.txt'})
...
async def test_read_file_numbers_lines_and_reports_the_file_hash(tmp_path: Path, workspace: Workspace) -> None:
    content = 'one\ntwo\n'
    (tmp_path / 'a.txt').write_text(content)

    result = await _call(_toolset(tmp_path), _ctx(workspace), 'read_file', {'path': 'a.txt'})

    assert result == f'[a.txt | 2 lines | hash:{_hash(content)}]\n     1\tone\n     2\ttwo\n'


async def test_read_file_pages_from_a_zero_based_offset(tmp_path: Path, workspace: Workspace) -> None:
    (tmp_path / 'lines.txt').write_text('one\ntwo\nthree\nfour\n')
    toolset = _toolset(tmp_path)

    first = await _call(toolset, _ctx(workspace), 'read_file', {'path': 'lines.txt', 'offset': 0, 'limit': 2})
    second = await _call(toolset, _ctx(workspace), 'read_file', {'path': 'lines.txt', 'offset': 2, 'limit': 2})
...
async def test_read_file_stops_at_max_read_lines_without_an_explicit_limit(
    tmp_path: Path, workspace: Workspace
) -> None:
...
async def test_read_file_reports_an_empty_file(tmp_path: Path, workspace: Workspace) -> None:
    (tmp_path / 'empty.txt').write_text('')

    result = await _call(_toolset(tmp_path), _ctx(workspace), 'read_file', {'path': 'empty.txt'})
...
async def test_read_file_rejects_an_offset_past_the_end_of_the_file(
    tmp_path: Path, workspace: Workspace, path: str
) -> None:
    (tmp_path / 'lines.txt').write_text('one\ntwo\n')
    (tmp_path / 'empty.txt').write_text('')

    with pytest.raises(ModelRetry, match='Offset 4 exceeds file length.'):
        await _call(_toolset(tmp_path), _ctx(workspace), 'read_file', {'path': path, 'offset': 4, 'limit': 1})
...
async def test_read_file_allows_a_symlink_alias_not_matching_a_denied_target_pattern(
    ...
    result = await _call(
        _toolset(tmp_path, denied_patterns=('*.secret',)), _ctx(workspace), 'read_file', {'path': 'alias.txt'}
    )
...
async def test_read_file_reports_a_missing_path_or_a_directory(
    tmp_path: Path, workspace: Workspace, path: str, message: str
) -> None:
    (tmp_path / 'directory').mkdir()

    with pytest.raises(ModelRetry, match=re.escape(message)):
        await _call(_toolset(tmp_path), _ctx(workspace), 'read_file', {'path': path})


# --- write_file ---


async def test_write_file_writes_content_and_reports_its_hash(tmp_path: Path, workspace: Workspace) -> None:
    content = 'ab\ncd\n'

    result = await _call(_toolset(tmp_path), _ctx(workspace), 'write_file', {'path': 'new.txt', 'content': content})

    assert result == f'Wrote 6 chars (2 lines) to new.txt. [hash:{_hash(content)}]'
    assert (tmp_path / 'new.txt').read_text() == content


async def test_write_file_overwrites_when_the_expected_hash_matches(tmp_path: Path, workspace: Workspace) -> None:
    before = 'before\n'
...
async def test_write_file_rejects_a_stale_expected_hash(tmp_path: Path, workspace: Workspace) -> None:
    before = 'before\n'
    (tmp_path / 'note.txt').write_text(before)
...
async def test_write_file_ignores_the_expected_hash_for_a_new_file(tmp_path: Path, workspace: Workspace) -> None:
    await _call(
        _toolset(tmp_path),
        _ctx(workspace),
        'write_file',
...
async def test_write_file_reports_a_target_it_cannot_write(
    ...
    with pytest.raises(ModelRetry, match=re.escape(message)):
        await _call(_toolset(tmp_path), _ctx(workspace), 'write_file', {'path': path, 'content': 'content'})


# --- edit_file ---


async def test_edit_file_replaces_the_single_match_and_reports_the_new_hash(
    tmp_path: Path, workspace: Workspace
) -> None:
...
async def test_edit_file_rejects_a_stale_expected_hash(tmp_path: Path, workspace: Workspace) -> None:
    before = 'before\n'
    (tmp_path / 'note.txt').write_text(before)
...
async def test_edit_file_reports_an_edit_it_cannot_apply(
    tmp_path: Path, workspace: Workspace, path: str, old_text: str, message: str
) -> None:
...
async def test_list_directory_reports_sizes_and_marks_directories(tmp_path: Path, workspace: Workspace) -> None:
    (tmp_path / 'a.txt').write_text('hello\n')
    (tmp_path / 'sub').mkdir()

    result = await _call(_toolset(tmp_path), _ctx(workspace), 'list_directory', {'path': '.'})

    assert result == 'a.txt  (6 bytes)\nsub/'


async def test_list_directory_reports_entries_relative_to_the_root(tmp_path: Path, workspace: Workspace) -> None:
    (tmp_path / 'sub').mkdir()
    (tmp_path / 'sub' / 'nested.txt').write_text('hello\n')

    result = await _call(_toolset(tmp_path), _ctx(workspace), 'list_directory', {'path': 'sub'})

    assert result == 'sub/nested.txt  (6 bytes)'


async def test_list_directory_reports_an_empty_directory(tmp_path: Path, workspace: Workspace) -> None:
    (tmp_path / 'empty').mkdir()

    result = await _call(_toolset(tmp_path), _ctx(workspace), 'list_directory', {'path': 'empty'})

    assert result == '(empty directory)'


async def test_list_directory_truncates_at_the_result_cap(tmp_path: Path, workspace: Workspace) -> None:
    for name in ('one.txt', 'three.txt', 'two.txt'):
        (tmp_path / name).write_text('x')

    result = await _call(_toolset(tmp_path, max_list_results=2), _ctx(workspace), 'list_directory', {'path': '.'})

    assert result.splitlines() == ['one.txt  (1 bytes)', 'three.txt  (1 bytes)', '[... truncated at 2 entries]']


async def test_list_directory_at_the_result_cap_is_not_marked_truncated(tmp_path: Path, workspace: Workspace) -> None:
...
async def test_list_directory_rejects_a_path_that_is_not_a_directory(
    ...
    with pytest.raises(ModelRetry, match=f'Not a directory: {path}'):
        await _call(_toolset(tmp_path), _ctx(workspace), 'list_directory', {'path': path})
...
async def test_search_files_reports_every_match_below_the_search_root(tmp_path: Path, workspace: Workspace) -> None:
    ...
    result = await _call(_toolset(tmp_path), _ctx(workspace), 'search_files', {'pattern': 'needle'})

    assert sorted(result.splitlines()) == ['a.txt:1:needle here', 'sub/nested.txt:2:needle there']


async def test_search_files_searches_only_below_the_given_path(tmp_path: Path, workspace: Workspace) -> None:
    (tmp_path / 'a.txt').write_text('needle here\n')
    (tmp_path / 'sub').mkdir()
    (tmp_path / 'sub' / 'nested.txt').write_text('needle there\n')

    result = await _call(_toolset(tmp_path), _ctx(workspace), 'search_files', {'pattern': 'needle', 'path': 'sub'})

    assert result == 'sub/nested.txt:1:needle there'


async def test_search_files_reports_no_matches(tmp_path: Path, workspace: Workspace) -> None:
...
async def test_search_files_keeps_only_files_matching_the_include_glob(tmp_path: Path, workspace: Workspace) -> None:
    (tmp_path / 'main.py').write_text('needle\n')
    (tmp_path / 'notes.md').write_text('needle\n')

    result = await _call(
        _toolset(tmp_path), _ctx(workspace), 'search_files', {'pattern': 'needle', 'include_glob': '*.py'}
    )

    assert result == 'main.py:1:needle'


async def test_search_files_skips_binary_files(tmp_path: Path, workspace: Workspace) -> None:
...
async def test_search_files_truncates_at_the_result_cap(tmp_path: Path, workspace: Workspace) -> None:
    (tmp_path / 'many.txt').write_text('needle\n' * 10)

    result = await _call(
        _toolset(tmp_path, max_search_results=2), _ctx(workspace), 'search_files', {'pattern': 'needle'}
    )

    assert result.splitlines() == ['many.txt:1:needle', 'many.txt:2:needle', '[... truncated at 2 matches]']


async def test_search_files_at_the_result_cap_is_not_marked_truncated(tmp_path: Path, workspace: Workspace) -> None:
    (tmp_path / 'many.txt').write_text('needle\nneedle\n')
...
async def test_find_files_matches_each_pattern_shape(
    tmp_path: Path, workspace: Workspace, pattern: str, expected: str
) -> None:
    (tmp_path / 'a.txt').write_text('')
    (tmp_path / 'sub').mkdir()
    (tmp_path / 'sub' / 'nested.py').write_text('')

    result = await _call(_toolset(tmp_path), _ctx(workspace), 'find_files', {'pattern': pattern})

    assert result == expected


async def test_find_files_marks_directories_with_a_trailing_slash(tmp_path: Path, workspace: Workspace) -> None:
    (tmp_path / 'sub').mkdir()

    result = await _call(_toolset(tmp_path), _ctx(workspace), 'find_files', {'pattern': 'sub'})

    assert result == 'sub/'


async def test_find_files_searches_only_below_the_given_path(tmp_path: Path, workspace: Workspace) -> None:
    (tmp_path / 'a.py').write_text('')
    (tmp_path / 'sub').mkdir()
    (tmp_path / 'sub' / 'nested.py').write_text('')

    result = await _call(_toolset(tmp_path), _ctx(workspace), 'find_files', {'pattern': '*.py', 'path': 'sub'})

    assert result == 'sub/nested.py'


async def test_find_files_reports_no_matches(tmp_path: Path, workspace: Workspace) -> None:
    (tmp_path / 'a.txt').write_text('')
...
async def test_find_files_rejects_an_absolute_pattern(tmp_path: Path, workspace: Workspace) -> None:
    with pytest.raises(ModelRetry, match=re.escape("Pattern '/etc/*' must be relative to the search path")):
        await _call(_toolset(tmp_path), _ctx(workspace), 'find_files', {'pattern': '/etc/*'})


@pytest.mark.parametrize('path', ['missing', 'file.txt'], ids=['missing', 'file'])
async def test_find_files_rejects_a_path_that_is_not_a_directory(
    tmp_path: Path, workspace: Workspace, path: str
) -> None:
    (tmp_path / 'file.txt').write_text('file')

    with pytest.raises(ModelRetry, match=f'Not a directory: {path}'):
        await _call(_toolset(tmp_path), _ctx(workspace), 'find_files', {'pattern': '*', 'path': path})


async def test_find_files_truncates_at_the_result_cap(tmp_path: Path, workspace: Workspace) -> None:
    for name in ('one.py', 'three.py', 'two.py'):
        (tmp_path / name).write_text('')

    result = await _call(_toolset(tmp_path, max_find_results=2), _ctx(workspace), 'find_files', {'pattern': '*.py'})

    assert result.splitlines() == ['one.py', 'three.py', '[... truncated at 2 matches]']


async def test_find_files_at_the_result_cap_is_not_marked_truncated(tmp_path: Path, workspace: Workspace) -> None:
...
async def test_create_directory_creates_missing_parents(tmp_path: Path, workspace: Workspace) -> None:
    result = await _call(_toolset(tmp_path), _ctx(workspace), 'create_directory', {'path': 'deep/nested'})

    assert result == 'Created directory: deep/nested'
    assert (tmp_path / 'deep' / 'nested').is_dir()


async def test_create_directory_accepts_a_directory_that_already_exists(tmp_path: Path, workspace: Workspace) -> None:
    (tmp_path / 'existing').mkdir()

    result = await _call(_toolset(tmp_path), _ctx(workspace), 'create_directory', {'path': 'existing'})
...
async def test_create_directory_reports_a_path_blocked_by_a_file(
    ...
    with pytest.raises(ModelRetry, match=re.escape(message)):
        await _call(_toolset(tmp_path), _ctx(workspace), 'create_directory', {'path': path})


# --- file_info ---


async def test_file_info_reports_size_lines_and_the_file_hash(tmp_path: Path, workspace: Workspace) -> None:
    content = 'first\nsecond\n'
    (tmp_path / 'info.txt').write_text(content)

    result = await _call(_toolset(tmp_path), _ctx(workspace), 'file_info', {'path': 'info.txt'})
...
async def test_file_info_omits_text_metadata_for_a_directory_or_binary_file(
    tmp_path: Path, workspace: Workspace, path: str, expected: str
) -> None:
    (tmp_path / 'directory').mkdir()
    (tmp_path / 'binary.bin').write_bytes(b'not utf-8: \xff')

    assert await _call(_toolset(tmp_path), _ctx(workspace), 'file_info', {'path': path}) == expected
...
async def test_file_info_looks_for_null_bytes_only_in_the_first_8kb(
    tmp_path: Path, workspace: Workspace, padding: int, binary: bool
) -> None:
    (tmp_path / 'padded.bin').write_bytes(b'x' * padding + b'\x00')

    result = await _call(_toolset(tmp_path), _ctx(workspace), 'file_info', {'path': 'padded.bin'})

    assert f'binary: {binary}' in result


async def test_file_info_reports_a_missing_path(tmp_path: Path, workspace: Workspace) -> None:
    with pytest.raises(ModelRetry, match='Path not found: missing'):
        await _call(_toolset(tmp_path), _ctx(workspace), 'file_info', {'path': 'missing'})
...
async def test_paths_resolving_outside_the_root_are_rejected(tmp_path: Path, workspace: Workspace, path: str) -> None:
    (tmp_path / 'sub').mkdir()
...
async def test_denied_patterns_reject_only_matching_paths(tmp_path: Path, workspace: Workspace) -> None:
    (tmp_path / 'creds.secret').write_text('hunter2\n')
    (tmp_path / 'notes.txt').write_text('notes\n')
    toolset = _toolset(tmp_path, denied_patterns=('*.secret',))

    assert 'notes' in await _call(toolset, _ctx(workspace), 'read_file', {'path': 'notes.txt'})
    with pytest.raises(ModelRetry, match=re.escape("Path 'creds.secret' is denied by pattern '*.secret'.")):
        await _call(toolset, _ctx(workspace), 'read_file', {'path': 'creds.secret'})


async def test_allowed_patterns_reject_paths_outside_the_allow_list(tmp_path: Path, workspace: Workspace) -> None:
    (tmp_path / 'main.py').write_text('code\n')
    (tmp_path / 'notes.md').write_text('notes\n')
    toolset = _toolset(tmp_path, allowed_patterns=('*.py',))

    assert 'code' in await _call(toolset, _ctx(workspace), 'read_file', {'path': 'main.py'})
    with pytest.raises(ModelRetry, match=re.escape("Path 'notes.md' does not match any allowed pattern.")):
        await _call(toolset, _ctx(workspace), 'read_file', {'path': 'notes.md'})
...
async def test_protected_patterns_reject_every_writing_tool(
    tmp_path: Path, workspace: Workspace, name: str, args: dict[str, object]
) -> None:
    (tmp_path / '.env').write_text('SECRET=abc\n')
    toolset = _toolset(tmp_path, protected_patterns=('.env', '.env/*'))

    with pytest.raises(ModelRetry, match='is protected'):
        await _call(toolset, _ctx(workspace), name, args)


async def test_protected_paths_stay_readable(tmp_path: Path, workspace: Workspace) -> None:
    (tmp_path / '.env').write_text('SECRET=abc\n')

    result = await _call(
        _toolset(tmp_path, protected_patterns=('.env',)), _ctx(workspace), 'read_file', {'path': '.env'}
    )

    assert 'SECRET=abc' in result


async def test_protected_patterns_leave_every_other_path_writable(tmp_path: Path, workspace: Workspace) -> None:
    await _call(
...
async def test_patterns_match_the_canonical_path(tmp_path: Path, workspace: Workspace) -> None:
    ...
    with pytest.raises(ModelRetry, match='is denied by pattern'):
        await _call(toolset, _ctx(workspace), 'read_file', {'path': 'config/./secret.txt'})


async def test_a_leading_double_star_pattern_also_matches_at_the_root(tmp_path: Path, workspace: Workspace) -> None:
    (tmp_path / 'secrets.yaml').write_text('api: key\n')
    toolset = _toolset(tmp_path, protected_patterns=('**/secrets*',))

    with pytest.raises(ModelRetry, match='is protected'):
        await _call(toolset, _ctx(workspace), 'write_file', {'path': 'secrets.yaml', 'content': 'changed\n'})
...
async def test_walkers_hide_denied_entries(
    tmp_path: Path, workspace: Workspace, name: str, args: dict[str, object], expected: str
) -> None:
    (tmp_path / 'visible.txt').write_text('marker\n')
    (tmp_path / 'creds.secret').write_text('marker\n')

    result = await _call(_toolset(tmp_path, denied_patterns=('*.secret',)), _ctx(workspace), name, args)
...
async def test_walkers_show_only_allowed_entries(
    ...
    result = await _call(_toolset(tmp_path, allowed_patterns=('*.txt',)), _ctx(workspace), name, args)
...
async def test_walkers_show_protected_entries(
    ...
    result = await _call(_toolset(tmp_path, protected_patterns=('*',)), _ctx(workspace), name, args)
...
async def test_walkers_hide_dotfiles(
    ...
    result = await _call(_toolset(tmp_path), _ctx(workspace), name, args)
...
@pytest.mark.parametrize(
    ('error', 'expected'),
    [
        (WorkspaceError('temporary failure'), ModelRetry),
        (WorkspaceUnavailableError('gone'), WorkspaceUnavailableError),
        (RuntimeError('backend bug'), RuntimeError),
    ],
    ids=['workspace-error', 'unavailable', 'runtime-error'],
)
@pytest.mark.parametrize(
    ('name', 'args'),
    [
...
async def test_a_path_name_that_is_too_long_is_recoverable(tmp_path: Path, workspace: Workspace) -> None:
    with pytest.raises(ModelRetry, match='The path name is too long.'):
        await _call(_toolset(tmp_path), _ctx(workspace), 'read_file', {'path': 'x' * 300})


async def test_a_symlink_loop_is_recoverable(tmp_path: Path, workspace: Workspace) -> None:
    (tmp_path / 'loop').symlink_to(tmp_path / 'loop')

    with pytest.raises(ModelRetry, match='The path resolves through a symlink loop.'):
        await _call(_toolset(tmp_path), _ctx(workspace), 'read_file', {'path': 'loop'})
...
async def test_os_errors_are_recoverable_only_when_the_model_can_correct_them(
    ...
    with pytest.raises(expected, match=re.escape(message)):
        await _call(_toolset(Path('.')), _ctx(Workspace(_ErrorBackend(error))), 'create_directory', {'path': 'sub'})
...
async def test_error_messages_never_expose_a_path_outside_the_root(filename: object, expected: str) -> None:
    workspace = Workspace(_ErrorBackend(_permission_error(filename)))

    with pytest.raises(ModelRetry, match=re.escape(expected)):
        await _call(_toolset(Path('.')), _ctx(workspace), 'file_info', {'path': 'a.txt'})
...
async def test_the_capability_configuration_reaches_the_tools(tmp_path: Path, workspace: Workspace) -> None:
    (tmp_path / 'creds.secret').write_text('hunter2\n')
