...
def test_sprites_live_is_scoped_to_sprites_changes() -> None:
    lines = _workflow_lines()
    changes_block = lines[lines.index('  changes:') : lines.index('  clai-test:')]
    detect_block = changes_block[changes_block.index('      - id: detect-sprites') :]
    ...
    assert "github.ref_type == 'tag' || needs.changes.outputs.sprites == 'true'" in _job_condition('sprites-live')


def test_sprites_live_skips_pull_requests_without_secrets() -> None:
    # SPRITE_TOKEN is a repository secret: the same fork and Dependabot guard as
    # `localstack-integration`, so those runs skip instead of failing.
    sprites = _job_condition('sprites-live').replace('outputs.sprites ', 'outputs.localstack ')
    assert sprites == _job_condition('localstack-integration')


def test_sprites_live_gates_the_aggregate_check_and_may_be_skipped() -> None:
    lines = _workflow_lines()

    needs = next(line for line in lines if line.strip().startswith('needs: [') and 'coverage' in line)
    assert 'sprites-live' in needs
    allowed = next(line for line in lines if 'allowed-skips:' in line)
    assert 'sprites-live' in allowed.split(':', 1)[1].replace(',', ' ').split()


def test_sprites_live_runs_the_live_tier_with_a_step_scoped_secret() -> None:
    lines = _workflow_lines()
    ...
    assert (
        '      - run: uv sync --locked --group dev --extra sprites' in lines[lines.index('  sprites-live:') : run_index]
    )
