...
Gating:
  ...
  * also requires a non-empty `DAYTONA_API_KEY`, so a CI run without the secret skips.
  * a module-scoped `anyio_backend` fixture keeps the shared Daytona client on one asyncio loop.
...
async def _owned(client: daytona.AsyncDaytona) -> AsyncGenerator[DaytonaSandboxBackend]:
    ...
    backend = DaytonaSandboxBackend(client=client, auto_stop_minutes=15)
