...
class TestCreate:
    ...
    async def test_e2b_error_becomes_a_start_failure(self, fake_e2b: FakeE2B) -> None:
        ...
        with pytest.raises(WorkspaceError, match='Could not start E2B sandbox: SandboxException: no capacity'):
            await started()

    async def test_auth_error_is_terminal(self, fake_e2b: FakeE2B) -> None:
        fake_e2b.create_error = fake_e2b.auth_type('bad key')
        with pytest.raises(WorkspaceUnavailableError, match='E2B rejected the credentials'):
            await started()
...
class TestConnect:
    ...
    async def test_attaching_uses_the_sdk_default_lifetime(self, fake_e2b: FakeE2B) -> None:
        ...
        await started(ref=WorkspaceRef(provider='e2b', id='sbx-keep'))
    ...
    async def test_other_connect_failures_are_recoverable(self, fake_e2b: FakeE2B) -> None:
        ...
        with pytest.raises(WorkspaceError, match='Could not connect to E2B sandbox') as exc:
            await started(ref=WorkspaceRef(provider='e2b', id='sbx-keep'))
...
class TestRun:
    ...
    async def test_run_failure_is_a_recoverable_sandbox_error(self, fake_e2b: FakeE2B) -> None:
        ...
        backend = await started()
        with pytest.raises(WorkspaceError, match='Command could not run in the sandbox: transient blip') as exc:
            await backend.run(['x'])
    ...
    async def test_a_non_e2b_failure_names_its_type(self, fake_e2b: FakeE2B) -> None:
        ...
        backend = await started()
        with pytest.raises(WorkspaceError, match='ValueError: connection reset'):
            await backend.run(['x'])
    ...
    async def test_a_timeout_on_a_live_sandbox_stays_recoverable(self, fake_e2b: FakeE2B) -> None:
        ...
        backend = await started()
        with pytest.raises(WorkspaceError, match='slow') as exc:
            await backend.run(['x'])
        assert isinstance(exc.value, WorkspaceError)
        assert not isinstance(exc.value, WorkspaceUnavailableError)
    ...
    async def test_a_failing_health_probe_preserves_the_original_error(self, fake_e2b: FakeE2B) -> None:
        ...
        backend = await started()
        with pytest.raises(WorkspaceError, match='slow'):
            await backend.run(['x'])
