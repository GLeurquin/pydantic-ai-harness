...
|---|---|---|---|
| `filesystem` | [FileSystem](filesystem.md) | defaults | always |
| `shell` | [Shell](shell.md) | defaults | always |
| `planning` | [Planning](planning.md) | defaults | always |
| `repo_context` | [Repo Context](repo-context.md) | the working directory | always |
| `pydantic_ai_docs` | [Pydantic AI Docs](pydantic-ai-docs.md) | defaults | always |
| `web_search` | Pydantic AI's `WebSearch` | DuckDuckGo fallback when the `duckduckgo` extra is installed | always |
| `skills` | [Skills](skills.md) | `SKILLS_DIRECTORY` (`.agents/skills`) | that directory exists |
| `code_mode` | [Code Mode](code-mode.md) | defaults | the `code-mode` extra is installed |
